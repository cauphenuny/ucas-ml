...
3. *预训练的重要性*：在大规模预训练的基础上，RoBERTa 携带的丰富先验知识是解决 *Cold Start* 和 *类别不平衡问题* 的关键。预训练使得模型能够更好地泛化到小样本场景。

4. *Accuracy 的欺骗性*：LSTM 的 58% Accuracy 背后是接近 0 的极端情感召回率；Transformer 模型的约 70% Accuracy 代表了更均衡、更可用的模型。
