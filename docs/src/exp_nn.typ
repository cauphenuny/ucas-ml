...
3. 值得注意的是，即使是 $5 × 10^(-5)$ 这一在 Base 模型上常用的学习率，在训练后期（约 5000 step 后）也出现了准确率急剧下滑的崩溃现象，进一步证明了 Large 模型对高学习率的容忍度极低。
...
2. *余弦衰减*：
   预热结束后，学习率按余弦曲线缓慢下降。相比恒定学习率（Constant），余弦衰减在训练后期能以更细微的步长进行参数微调，有效抑制了验证集上的波动。
