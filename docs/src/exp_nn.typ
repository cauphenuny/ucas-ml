...
#figure(image("assets/exp/lr/roberta.png", width: 50%))

这种现象被称为类别塌陷或模型退化。原因在于：
1. `RoBERTa-Large` 拥有 3.5 亿参数，其预训练权重已处于高度优化的极小值区域。过大的更新步长会产生“灾难性遗忘”（Catastrophic Forgetting），破坏模型已掌握的语义提取能力。
2. 由于随机初始化的分类头在初期会产生巨大的梯度，配合高学习率，这股冲击力会直接传导至底层权重，导致模型为了快速降低 Loss 而选择“全输出单一高频标签”的局部最优解。
3. 值得注意的是（见图 3 曲线），即使是 $5 × 10^(-5)$ 这一在 Base 模型上常用的学习率，在训练后期（约 5000 step 后）也出现了准确率急剧下滑的崩溃现象，进一步证明了 Large 模型对高学习率的容忍度极低。
...
为了平衡训练早期的稳定性和后期的精准度，我们引入了带预热（Warmup）的调度机制。

1. *线性预热*：
   在前 6% 的步数中，学习率从 0 线性增加至设定值（如 $2 × 10^(-5)$）。这为随机初始化的分类头提供了“缓冲期”，避免初始梯度冲击破坏底座权重。

2. *余弦衰减*：
   预热结束后，学习率按余弦曲线缓慢下降（见图 1）。相比恒定学习率（Constant），余弦衰减在训练后期能以更细微的步长进行参数微调，有效抑制了验证集上的波动（对比图 2 与图 3 的震荡程度）。
   - *Constant 表现*：即便在 $1 × 10^(-5)$ 下，准确率曲线仍存在明显锯齿。
   - *Cosine 表现*：曲线更加平滑，且最终能触及更高的精度上限。
